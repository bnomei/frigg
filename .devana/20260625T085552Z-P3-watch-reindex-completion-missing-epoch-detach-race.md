...
- 2026-06-27: reopened after fixed-status audit. The stale-completion/drop-refresh branch is blocked by the epoch token, but the original report also covered the duplicate concurrent manifest reindex branch, and that branch remains reachable. `WatchSchedulerState::remove_repository` still clears `in_flight_manifest_fast` while the detached lifecycle's `spawn_blocking` task can continue; a re-attach can queue a fresh `ManifestFast`, and the supervisor dispatch path calls `scheduler.mark_started` and starts `RuntimeTaskKind::ChangedReindex` without checking for an active changed-reindex task for the same repository. Scope for the remaining fix: preserve/observe in-flight manifest work across detach/re-attach, or add an active-task guard equivalent to the semantic-followup alias guard before starting `ManifestFast`.
- 2026-06-27: fixed. Added a refresh-task active guard before `scheduler.mark_started` for every ready watch refresh. For `ManifestFast`, the guard checks active `RuntimeTaskKind::ChangedReindex`; for `SemanticFollowup`, it keeps checking active `RuntimeTaskKind::SemanticRefresh`; both checks use the runtime repository id plus the stable root-hash id as aliases. If an active task is found (including an old detached lifecycle's blocking changed reindex), the scheduler calls `mark_failed` with retry/backoff instead of draining paths or starting a duplicate writer. Added regression coverage for the remaining branch: an active changed reindex defers a ready `ManifestFast`, leaves it pending, suppresses immediate redispatch, and allows it to run once the task finishes and the retry deadline elapses; alias coverage verifies stable-id semantic refresh still blocks semantic followup without blocking manifest-fast work.

DEVANA-KEY: crates/cli/src/watch/supervisor.rs:485 | P3 | watch-reindex-completion-missing-epoch-detach-race
DEVANA-SUMMARY: Status=fixed | P3 medium crates/cli/src/watch/supervisor.rs:485 - Epoch-gated completions block stale lifecycle mutations, and ready ManifestFast refreshes now defer while an active ChangedReindex exists for the same runtime/stable repository alias set.
