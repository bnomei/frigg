...
use super::ProjectedGraphContext;
use super::{
...
pub(in crate::searcher) struct HybridGraphArtifactCacheKey {
    pub(in crate::searcher) repository_id: String,
    pub(in crate::searcher) snapshot_id: Option<String>,
    candidate_signature: String,
    artifact_version: u32,
}

#[derive(Debug, Clone)]
pub(in crate::searcher) struct HybridGraphArtifact {
...
pub(in crate::searcher) fn search_graph_channel_hits(
...
fn hybrid_graph_artifact(
    ...
    let cache = match searcher.hybrid_graph_artifact_cache.read() {
        Ok(cache) => cache,
    ...
    let mut cache = match searcher.hybrid_graph_artifact_cache.write() {
        Ok(cache) => cache,
        Err(poisoned) => poisoned.into_inner(),
    };
