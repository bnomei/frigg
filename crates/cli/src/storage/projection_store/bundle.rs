...
use crate::storage::{
    RetrievalProjectionBundle, RetrievalProjectionHeadRecord, Storage, StorageSession,
...
impl Storage {
    ...
    pub fn stale_or_missing_retrieval_projection_families_for_repository_snapshot(
        ...
        let conn = self.open_current_schema_connection()?;
        stale_or_missing_retrieval_projection_families_on_connection(
            &conn,
            repository_id,
            snapshot_id,
            expected_versions,
...
fn stale_or_missing_retrieval_projection_families_on_connection(
