...
impl Storage {
    ...
    pub fn delete_snapshot(&self, snapshot_id: &str) -> FriggResult<()> {
        ...
        delete_snapshot_rows_in_transaction(&tx, snapshot_id)?;
...
fn upsert_repository_on_connection(
    conn: &Connection,
    repository_id: &str,
    root_path: &std::path::Path,
    display_name: &str,
...
fn upsert_manifest_on_connection(
    conn: &mut Connection,
    repository_id: &str,
    snapshot_id: &str,
    entries: &[ManifestEntry],
...
pub(super) fn delete_snapshot_rows_in_transaction(
    tx: &Transaction<'_>,
    snapshot_id: &str,
) -> FriggResult<()> {
    ...
    for (table, description) in [
        ("path_witness_projection", "path witness projection rows"),
        ("test_subject_projection", "test subject projection rows"),
        (
            "entrypoint_surface_projection",
            "entrypoint surface projection rows",
        ),
        (
            "retrieval_projection_head",
            "retrieval projection head rows",
        ),
        ("path_relation_projection", "path relation projection rows"),
        (
            "subtree_coverage_projection",
            "subtree coverage projection rows",
        ),
        (
            "path_surface_term_projection",
            "path surface-term projection rows",
        ),
        (
            "path_anchor_sketch_projection",
            "path anchor sketch projection rows",
        ),
        ("file_manifest", "manifest rows"),
    ] {
        tx.execute(
        ...
        .map_err(|err| {
            FriggError::Internal(format!(
                "failed to delete {description} for snapshot '{snapshot_id}': {err}"
            ))
        })?;
    }
