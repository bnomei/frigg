...
- `document_symbols` is deterministic and read-only for Rust, PHP, and Blade files only; unsupported extensions fail as typed `invalid_params`.
...
- `search_structural` is deterministic tree-sitter query search for Rust, PHP, and Blade only; `query` must be non-empty and at most `4096` characters, `language` (if provided) must be `rust`, `php`, or `blade`, and `path_regex` must satisfy safe-regex validation.
